...
{
	return "SELECT * FROM [DB_PROJET_POO].[dbo].[TB_STOCK]";
}
System::String^ NS_Comp_Mappage::CLmapStock::InsertStock(void)
{
	return "INSERT INTO TB_STOCK (st_prod_quantite, st_seuil_reap, ar_id) VALUES('" + this->st_prod_quantite + "', '" + this->st_seuil_reap + "', '" + this->ar_id + "'); ";
}
...
{
	return "UPDATE TB_PEOPLE SET st_prod_quantite = '" + this->st_prod_quantite + "', st_seuil_reap = '" + this->st_seuil_reap + "', ar_id = '"+ this->ar_id +"' WHERE st_id = " + "'" + this->id_stock + "'; ";
}
